/* ============================================================
   quotes.js — Marquee quote ticker + Qtype system
   Load ORDER: 7th (depends on: utils.js, themify.js)
   Activates only when #quoteWrapper + #quoteBox are present.

   Qtypes:
     Qleft    — right-to-left scroll (default, original behaviour)
     Qright   — left-to-right scroll
     Qrefresh — centered, no scroll, one random quote per page load
   ============================================================ */
...
(function injectQuoteStyles() {
  ...
  style.textContent = `
    ...
    /* Qrefresh: centered static layout */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let refreshQuote = null; // single cached quote for Qrefresh, lives for the page session
  ...
  function cycleQtype() {
    ...
    // Clear cached refresh-quote only when leaving Qrefresh
  ...
  // Expose globally so index.html onclick can call it
  window.WS_Quotes = { cycleQtype };

  // Also wire the button directly in case it's already in the DOM
  ...
  function parseQuoteWithImages(text) {
    ...
    out = out.replace(
      ...
      (_, rawProps, inner) => {
        ...
        if (src) startAudio(src, isNaN(vol) ? 1 : clamp(vol, 0, 1), startSec, endSec || undefined);
  ...
  const setQuote = () => {
    ...
    if (currentQtype === "Qrefresh") {
      // Pick once per page load (or first switch into Qrefresh); cache it.
    ...
    if (currentQtype === "Qrefresh") {
      ...
      if (currentQtype === "Qright") {
        ...
        // Qleft: start just off the right edge (original behaviour)
  ...
  /* ══════════════════════════════════════════════════════════
     animate — Qtype-aware rAF loop
     ══════════════════════════════════════════════════════════ */

  const animate = (time) => {
    if (currentQtype !== "Qrefresh" && _quoteReady && lastTime !== null) {
      if (currentQtype === "Qright") {
        // LTR: move right; cycle when fully past right edge
        pos += currentSpeed * ((time - lastTime) / 1000);
        box.style.transform = `translateX(${pos}px)`;
        if (pos > wrapper.offsetWidth) { stopAudio(); setQuote(); }
      } else {
        // Qleft (default): move left; cycle when fully past left edge
        pos -= currentSpeed * ((time - lastTime) / 1000);
        box.style.transform = `translateX(${pos}px)`;
