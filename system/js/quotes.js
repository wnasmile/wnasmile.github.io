...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Shared fallback walker — registered once on window so onerror="" inline attrs can call it.
  ...
  function buildStickerImg(path, ext) {
    if (path.includes("..")) return `:${path}:`; // block path traversal
  ...
  function prepareAudio(src, vol, startSec, endSec) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (_audFetchCtrl !== ctrl) { resolve(); return; } // cancelled by stopAudio()
        _audFetchCtrl = null;
  ...
  // Cache live DOM refs once — avoids repeated getElementById inside the rAF loop.
  ...
  function startPanVolumeAnimation() {
    ...
    const animatePanVol = () => {
      ...
      // Hard-silence when muted; otherwise apply normal spatial fade.
