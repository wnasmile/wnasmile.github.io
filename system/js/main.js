...
<body>
  ...
  function createAssetCards(data) {
    ...
    // Active page: use Promise.allSettled so a single broken image can't block reveal.
    // Off pages: settle independently so they don't slow each other or page 1.
    for (const [pageNum, { promises, cards }] of pagePromiseMap) {
      const isActive = pageNum === activePage;

      // Settle each card individually so fast-loading images appear right away.
      // Use a shared counter to trigger renderPage only on the last settled card.
      let settled = 0;
      ...
      cards.forEach((card, i) => {
        promises[i].finally(() => {
          ...
          }, isActive ? i * 30 : i * 60); // active page animates faster
        });
      });

      // Fire loader completion as soon as active page's images are all settled.
      if (isActive) {
        Promise.allSettled(promises).then(() => {
          if (!activePageHandled) { activePageHandled = true; runLoaderSequence(); }
        });
      }
    }

    // No images on the active page at all (empty page or all filtered out) — still
    // need to dismiss the loader so it doesn't hang.
    ...
    window.renderPage = () => {
      ...
      for (const [pageNum, cards] of window._cardIndex) {
        ...
        for (const c of cards) {
          // Non-ready cards: only show if on the active page (they stay none otherwise).
          // Ready cards: show on active page, hide on all others.
          // In both cases display:none is the safe default — cards start hidden at
          // creation so this only ever promotes a card to flex, never leaks it.
          const isReady   = c.classList.contains("ready");
          const want      = onThisPage && isReady && c.dataset.filtered === "true" ? "flex" : "none";
