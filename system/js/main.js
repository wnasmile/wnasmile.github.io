...
<body>
  ...
  function createAssetCards(data) {
    ...
    for (const asset of domOrdered) {
      ...
      // "new"/"updated" animated gif badges require config.gifBase to be set.
      // Skipped here — add gifBase to config in main.js initElements() to re-enable.
      // if (["new", "updated"].includes(status))
      //   addOverlay(wrapper, `${config.gifBase}${status}.gif`, ...);
      ...
      Promise.all(promises).then(() => {
        ...
        cards.forEach((card, i) => {
          setTimeout(() => {
            ...
            // Only call renderPage on the final card of this page batch.
            // renderPage will correctly show only the current page's ready cards
            // and keep everything else display:none — no intermediate renders needed.
            if (i === last && typeof window.renderPage === "function") window.renderPage();
          }, i * 40);
        });
        // Fire the loader completion sequence as soon as the active page's images
        // are all resolved — don't wait for off-page batches.
